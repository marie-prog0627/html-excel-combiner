...
function combiner(encd) {
    var myFile = document.getElementById("import-html").files[0];
    if (myFile.type.indexOf("text") == 0) {
        var reader = new FileReader();
        reader.onload = function (evt) {
            var separator = "table-inner";
            var txt = evt.target.result; //ファイル内容を読み出し
            txt = txt.split(separator); //split before div
            
            //change
            var content = txt[0];
            for(var i = 0;i < table.length;i++){
                content = content + table[i] + txt[i];
            }
            //finish change
